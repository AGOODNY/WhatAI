...
from .poetry_services import respond_to_poetry_game, start_poetry_game
...
class PoetryStartView(APIView):
    ...
    def post(self, request):
        ...
        if not persona:
            return Response({"error": "请选择有效的 AI 人格"}, status=400)
        return Response(start_poetry_game(
            persona=persona,
            user=request.user,
            history=request.data.get("history", []),
        ))
...
class PoetryRespondView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        ...
        if not persona:
            return Response({"error": "请选择有效的 AI 人格"}, status=400)
        try:
            result = respond_to_poetry_game(
                persona=persona,
                user=request.user,
                game_token=request.data.get("game_token"),
                raw_verses=request.data.get("verses"),
                history=request.data.get("history", []),
                action=request.data.get("action", "chat"),
                message=str(request.data.get("message", ""))[:1000],
                timed_out=request.data.get("timed_out"),
